...
public class ActionRequestSender : NetworkSingleton<ActionRequestSender>
{
    ...
    {
        if (!CanDoAction(rpcParams)) return;
        List<IPacket> clientEffects = GameServerAction.LaunchSpell(spellId, targetPos, GameManagerServer.Instance.GameState);
    ...
    {
        if (!CanDoAction(rpcParams)) return;
        IPacket packet = GameServerAction.NextTurn(GameManagerServer.Instance.GameState);
        ActionResultSender.Instance.SendEffectClientRpc(MessagePackSerializer.Serialize(packet));
